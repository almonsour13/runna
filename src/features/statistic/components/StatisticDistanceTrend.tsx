...
export default function StatisticDistanceTrend() {
    ...
    const barData = useMemo(() => {
        ...
        if (activeTab === "Year") {
            ...
            activities.forEach((a) => {
                ...
                byMonth.set(key, (byMonth.get(key) ?? 0) + (a.distance ?? 0));
            });
            return months.map((date) => ({
                date,
                distance: byMonth.get(format(date, "yyyy-MM")) ?? 0,
    ...
    const getBarLabel = (idx: number, date: Date): string | false => {
        ...
        if (activeTab === "Year") return format(date, "MMM")[0]; // J, F, M, A...
        return false;
    };
                        ...
                        <RowView className="gap-2">
                            ...
                            >
                                {ruler.map((r, i) => (
                                    ...
                                    >
                                        {
                                            formatDistanceByUnit(r, unit, false)
                                                .value
                                        }
                                    </Text>
                                ))}
